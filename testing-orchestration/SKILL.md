...
**Regla de redacción al consolidar aprendizajes (workflow M)**: los invariantes y casos reales se escriben AGNÓSTICOS — la forma del fallo y su porqué, sin nombres de proyecto, IDs de bugs, versiones ni símbolos propietarios (un lector de otro proyecto debe poder aplicarlos tal cual). El detalle específico vive en el bug tracker y los docs del proyecto que originó el aprendizaje; las referencias legítimas a material del proyecto van vía el peer doc (`CLAUDE.md` y las variables de adopción), nunca en el cuerpo del invariante.
...
27. **Descubrimiento de código — el grafo NO es una preferencia, es el método; `grep` sobre código es fallback de urgencia y se declara como tal.** Siempre que se **audite, diagnostique o modifique CÓDIGO** —auditoría, bug, hotfix, feature nueva, refactor, revisión de tests, inventario de llamadores del invariante 37— la exploración se hace con el MCP de índice/grafo (`codebase-memory-mcp`: `search_graph`, `trace_path`, `get_code_snippet`, `query_graph`, `get_architecture`). No es "la primera opción recomendada": es **la que responde la pregunta**. Si el proyecto no está indexado, `index_repository` **primero**; si el índice está frío, refrescarlo cuesta menos que el error que evita.
    ...
    **La delegación NO hereda esta disciplina sola — se escribe en el prompt (aprendizaje 2026-09-08)**: al encargar a un subagente cualquier auditoría, inventario o diagnóstico de código, su prompt debe exigir EXPLÍCITAMENTE el método completo — (a) el índice `codebase-memory-mcp` con el nombre del proyecto y su estado de frescura, (b) grep AMPLIO, y (c) el CONTRASTE de ambos resultados antes de concluir, declarando la cobertura usada en cada afirmación. Mencionar el índice como opción ("úsalo si lo necesitas") no basta: el agente optimiza por velocidad y se queda con una sola fuente, que es exactamente el inventario incompleto con apariencia de completo que este invariante existe para impedir. La discrepancia entre las dos fuentes no es ruido a resolver en silencio: ES un hallazgo (caso real: sobre el CRUD de un repositorio base consumido por herencia, el grafo resolvía 4/30/9 llamadores frente a 26/44/13 por grep — la diferencia era la herencia sin resolver, y quien mirara solo el grafo habría clasificado un tercio del inventario).
...
37. **Al cambiar un invariante GLOBAL, el inventario de lo que depende de él se hace ANTES y se trata en el MISMO commit — incluidos los tests que verificaban la regla anterior.** Cambiar un invariante global es añadir un guard que aborta, sustituir un camino por otro, o restringir una configuración a menos valores de los que admitía. Lo que los hace peligrosos es que su alcance **no es el fichero que estás editando**: es el conjunto de puntos que asumían la regla vieja. La pregunta obligatoria antes de cerrar el commit es una sola: *¿qué otros puntos dependen del invariante que estoy cambiando, y qué tests lo verifican?* Se responde con el **grafo de llamadas** (invariante 27), no de memoria ni a golpe de grep.
    ...
    **El inventario alcanza también al ESTADO que decides NO cambiar, y el grafo de llamadas no enseña esa dependencia** (aprendizaje 2026-09-10). Caso real: una acción de operador para devolver el acceso a un cliente se extrajo del carril "disputa ganada" a un punto único, con la decisión deliberada — y correcta — de no tocar el estado del expediente (el resync de la pasarela pisaba cualquier estado manual). Pero en el carril original, la liberación de los créditos retenidos ocurría GRATIS: no era código del carril, sino un gate transversal en otro dominio —una query que retiene mientras el expediente siga "vivo"— que dejaba de morder como efecto lateral del cambio de estado. Al extraer el código, la mitad implícita del efecto se quedó atrás: la acción devolvía membresía y cobro, y el cliente seguía sin poder gastar. Nada lo delató — los tests verdes afirmaban lo que el código extraído hacía, y `trace_path` no tiene arista hacia un filtro de query que depende de un estado. Dos reglas salen de aquí: (a) al decidir que una operación NO cambia un estado, la pregunta del inventario se invierte — *¿qué gates, filtros o queries transversales dependen de que ese estado cambie?* — y se responde buscando los LECTORES del estado (search_graph/grep por el campo y su semántica), no los llamadores del código; (b) el contrato de una acción de operador se inventaría por DESENLACES OBSERVABLES desde la persona afectada ("¿qué cambia para el cliente al pulsar esto?"), no por los efectos del código extraído — si el nombre de la acción promete un resultado, cada componente observable de ese resultado necesita su aserción, y el que falte es exactamente el que un test derivado del código jamás cubrirá. Lo destapó la validación manual por UI con el escenario completo — la razón de ser de intercalar esa fase.
...
40. **El trabajo se hace en el checkout PRINCIPAL del proyecto — nunca en un git worktree aparte ni en un clon paralelo.** La tentación es razonable: una batería larga de fixes "ensucia" el árbol, y aislarla en un worktree parece higiénico. Lo que ese razonamiento no ve es **quién más está mirando ese directorio**. El stack de desarrollo (`up:dev`) sirve el código del checkout principal por bind-mount, y el usuario prueba sus casos **en paralelo** a la edición, contra esa misma URL. Mover el trabajo a otro directorio no aísla nada: deja al usuario probando el código de antes mientras los cambios viven donde su navegador no llega — y los defectos que reporte entonces describen una versión que ya no existe. El aislamiento que sí sirve es el de **git**: una rama en este mismo directorio, PR y merge al terminar. La restricción es sobre el **directorio de trabajo**, no sobre el uso de ramas. Aplica igual al delegar: un subagente con `isolation: "worktree"` produce el mismo desfase, agravado por que sus cambios pueden no volver nunca. En este proyecto está además cerrado por configuración (`EnterWorktree` denegada en `.claude/settings.json`), pero la razón por la que está cerrado importa más que el guard: **el entorno de pruebas del usuario y el de edición son el mismo, por diseño**.


41. **El gate de cierre de toda FASE o PR con cambios de comportamiento incluye `test:unit` COMPLETO — las suites E2E/API no lo sustituyen.** La tentación es razonable: si el `test:stripe` completo y los módulos de API están en verde, el trabajo "está cubierto". No lo está, y la razón es de reparto de responsabilidades: los E2E validan el camino del usuario y los eventos reales del proveedor, pero los contratos internos finos —el ancla FK de un débito, la atribución de un forfeit a su período, un cálculo con dos períodos del mismo plan— viven en tests unit que fabrican combinaciones que ningún camino E2E recorre en esa forma exacta. Un rojo unit puede convivir durante días con toda la batería E2E en verde.

    **Caso real (2026-09-08)**: tres fases consecutivas de un mismo plan cerraron cada una con la suite E2E completa del proveedor + los módulos de API + los tests de background en verde — y ninguna corrió la suite unit completa. Dos tests unit de un contrato de dinero llevaban ROJOS desde el segundo commit de la primera fase: un guard introducido allí cambió cuándo dejaban de estar "activas" unas filas, y el cálculo que dependía de leerlas activas después empezó a anclar el débito al período equivocado — importe corto y atribución errónea. La regresión atravesó TRES releases, llegó a desplegarse, y se descubrió por casualidad: la primera pasada unit completa de la fase siguiente. El bisect commit a commit la acotó en minutos; el gate la habría parado el primer día. (El detalle con nombres vive en el bug tracker del proyecto — la skill lleva la FORMA del fallo.)
...
- **Todo bug o gap que se ABRE lleva su test que lo DESTAPA** (petición del usuario 2026-09-08): un test en ROJO que captura el hecho sorprendente por el camino real (endpoint), marcado `pending_bug` con el ID del bug en su docstring, y la entrada del tracker referencia el test POR NOMBRE. Por qué: una entrada sin test es una descripción que envejece — nadie sabrá después si el hecho sigue ocurriendo, y al abordar el bug habrá que redescubrirlo; el test `pending_bug` mantiene el hecho VIVO (corre en `test:bugs`, no rompe la pipeline) y al arreglar el bug se desmarca y se convierte en la red del fix. Si el bug es un GAP con decisión de producto pendiente, el test afirma la expectativa más razonable del operador y su docstring declara que se SUSTITUIRÁ por el comportamiento decidido — su trabajo es sostener el hecho hasta que la decisión exista. Excepción: entradas de pura deuda de diseño sin comportamiento observable hoy (no hay hecho que destapar) — se declara en la entrada.
