...
27. **Descubrimiento de código — grafo/índice del código SIEMPRE por encima de la exploración textual directa**. Si el proyecto expone un MCP de índice/grafo de código (p.ej. `codebase-memory-mcp`) conectado e indexado (`index_status` = `ready`), es la **primera opción** para explorar/entender CÓDIGO: símbolos por nombre/label/qualified-name, call chains / data-flow / cross-service, fuente exacta por qualified name, arquitectura. Se usa **por encima** de `grep`/`Read`/`Explore` directos — que quedan para **TEXTO, configs, i18n, docs y ficheros no-code** (su punto fuerte) y como **fallback** cuando el MCP no está disponible (clones sin indexar, headless). Da el grafo de llamadas real, sin los falsos positivos del grep; si el proyecto no está indexado, `index_repository` primero. Detalle y guard "si está disponible" en el peer doc (`CLAUDE.md §5 "Búsqueda de código"`).
